...
<div class="fmm-item-row" data-id="{{ $item['id'] }}">
    ...
    <div class="fmm-item-card {{ !$item['enabled'] ? 'disabled-item' : '' }}">
        ...
        <div class="fmm-item-body">
            <div style="display:flex;align-items:center;gap:0.5rem;flex-wrap:wrap">
                ...
                {{-- ===== TAMBAHAN: Preview Icon ===== --}}
                @if (!empty($item['icon']))
                    <i class="{{ $item['icon'] }}"
                        style="font-size:0.8rem;color:var(--fmm-muted);width:1rem;text-align:center"></i>
                @endif
                {{-- ===== END TAMBAHAN ===== --}}
                ...
                {{-- ===== TAMBAHAN: Badge Style Spesial ===== --}}
                @if (!empty($item['data']['style']))
                    <span class="fmm-item-badge" style="background:rgb(219 234 254);color:rgb(29 78 216);">
                        {{ $item['data']['style'] }}
                    </span>
                @endif
                {{-- ===== END TAMBAHAN ===== --}}
        ...
        <div class="fmm-item-actions">
            ...
            <button class="fmm-action-btn" wire:click="moveDown({{ $item['id'] }})" title="Move down">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"
                    style="width:0.875rem;height:0.875rem">
                    <path fill-rule="evenodd"
                        d="M5.22 8.22a.75.75 0 000 1.06l4.25 4.25a.75.75 0 001.06 0l4.25-4.25a.75.75 0 00-1.06-1.06L10 11.94 6.28 8.22a.75.75 0 00-1.06 0z"
                        clip-rule="evenodd" />
                </svg>
            </button>
            ...
            {{-- Outdent (←) --}}
            @if ($depth > 0)
                <button class="fmm-action-btn" wire:click="outdentItem({{ $item['id'] }})"
            ...
            {{-- Toggle visibility --}}
            <button class="fmm-action-btn" wire:click="toggleEnabled({{ $item['id'] }})"
                ...
                @else
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"
                        style="width:0.875rem;height:0.875rem;opacity:0.4">
                        <path fill-rule="evenodd"
    ...
    @if (isset($editingItemId) && $editingItemId == $item['id'])
        <div class="fmm-edit-form" style="margin-top:0.375rem">
            ...
            {{-- ===== TAMBAHAN: Icon & Style di form edit ===== --}}
            <div class="fmm-edit-row">
                <div style="flex:2">
                    <div class="fmm-label">Icon <span style="font-weight:400;color:var(--fmm-muted)">(FontAwesome
                            class)</span></div>
                    <input type="text" wire:model="editingIcon" placeholder="e.g. fas fa-home" />
                    @if (!empty($editingIcon))
                        <div style="margin-top:0.25rem;font-size:0.75rem;color:var(--fmm-muted)">
                            Preview: <i class="{{ $editingIcon }}"></i>
                        </div>
                    @endif
                </div>
                <div style="flex:1">
                    <div class="fmm-label">Style</div>
                    <select wire:model="editingStyle">
                        <option value="">Default</option>
                        <option value="btn-permohonan">Tombol Permohonan</option>
                    </select>
                </div>
            </div>
            {{-- ===== END TAMBAHAN ===== --}}
            ...
            <div style="display:flex;align-items:center;gap:0.5rem">
                <input type="checkbox" wire:model="editingEnabled" id="enabled-{{ $item['id'] }}"
                    style="width:auto" />
                <label for="enabled-{{ $item['id'] }}" class="fmm-label"
                    style="margin:0;cursor:pointer">Visible</label>
            </div>
            <div class="fmm-edit-actions">
                <button class="fmm-action-btn" wire:click="cancelEdit"
                    style="width:auto;padding:0.3rem 0.75rem;font-size:0.8rem;border:1.5px solid var(--fmm-border)">Cancel</button>
                <button wire:click="saveEdit"
                    style="padding:0.3rem 0.875rem;font-size:0.8rem;font-weight:600;background:var(--fmm-accent);color:#fff;border:none;border-radius:0.375rem;cursor:pointer">Save</button>
            </div>
        </div>
    @endif
    ...
    @if (!empty($item['children']))
        <div class="fmm-nested-list fmm-nested-sortable" x-data="menuSortable($wire, {{ config('filament-menu-manager.max_depth') ?? 'null' }})">
            @foreach ($item['children'] as $childIdx => $child)
                @include('filament-menu-manager::components.menu-item', [
                    'item' => $child,
                    'depth' => $depth + 1,
                    'index' => $childIdx,
                ])
