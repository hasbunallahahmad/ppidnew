...
class StorePermohonanRequest extends FormRequest
{
  ...
  {
    $this->merge([
      'nama_pemohon' => trim($this->input('nama_pemohon')),
      'email' => strtolower(trim($this->input('email'))),
      'no_telepon' => preg_replace('/\s+/', '', $this->input('no_telepon')), // Remove spaces
      'alamat' => trim($this->input('alamat')),
      'no_identitas' => preg_replace('/[^0-9]/', '', $this->input('no_identitas')), // Only numbers
      'informasi_diminta' => trim($this->input('informasi_diminta')),
      'tujuan_penggunaan' => trim($this->input('tujuan_penggunaan')),
    ]);
  }
}
